...
export const userLogin = async (email: string, password: string): Promise<types.AuthenticatedUser> => {
...
export const getPaginatedBooks = async (page = 1, itemsPerPage = 25, authorizationToken: string): Promise<types.Book[]> => {
	const response = await axiosBase.get<types.Book[]>(`/books?page=${page}&amount=${itemsPerPage}`,
		{
			headers: {
